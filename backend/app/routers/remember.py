...
@router.post("/", response_model=RememberResponse, status_code=status.HTTP_201_CREATED, 
             summary="Remember a memory for the authenticated project")
async def remember(
    request: RememberRequest, project: Project = Depends(get_current_project), 
    db: AsyncSession = Depends(get_db),) -> RememberResponse:
