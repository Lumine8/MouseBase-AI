...
class SearchService:
    ...
    async def search(self, project: Project, request: SearchRequest) -> SearchResponse:
        ...
        stmt = (
            select(Memory, distance.label("distance")).join(Embedding, Embedding.memory_id == Memory.id)
            .where(Memory.project_id == project.id, Embedding.model == settings.EMBEDDING_MODEL).order_by(distance)
            .limit(request.top_k)
        ) 
